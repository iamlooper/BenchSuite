...
@Composable
fun LeaderboardCategoryDetailScreen(
    categoryId: String,
    onBack: () -> Unit,
    viewModel: LeaderboardCategoryDetailViewModel = hiltViewModel(),
) {
    ...
    AppScaffold(
        ...
        when (val state = uiState) {
            LeaderboardCategoryDetailUiState.Loading -> {
                Box(
                    modifier = Modifier
                        .padding(innerPadding)
                        .fillMaxSize(),
            ...
            is LeaderboardCategoryDetailUiState.Success -> {
                val benchmarks = state.categoryScore.benchmarks
